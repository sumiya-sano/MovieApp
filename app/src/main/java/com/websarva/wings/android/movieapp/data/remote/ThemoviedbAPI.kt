...
interface ThemoviedbAPI {
    ...
    @GET("/3/search/movie")
    //引数でクエリを注入
    suspend fun keywordSearchMovies(@Query("query") query: String): KeywordSearchMovieDto
    ...
    //引数でクエリを注入
    suspend fun searchMovieDetail(@Path("movie_id") movieId: Int): SearchMovieDetailDto
}